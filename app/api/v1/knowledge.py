...
async def create_knowledge(payload: KnowledgeCreate, db: DbSession, _: CurrentUser):
    ...
    return {"id": entry.id, "content": entry.content, "source_script_id": entry.source_script_id, "created_at": str(entry.created_at)}
...
async def list_knowledge(db: DbSession, _: CurrentUser, limit: int = Query(default=50, ge=1, le=200)):
    ...
    return [{"id": e.id, "content": e.content, "source_script_id": e.source_script_id, "created_at": str(e.created_at)} for e in entries]
