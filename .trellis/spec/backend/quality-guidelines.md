...
- Under `write_allowed`, `api_runner` may execute planner-provided `api.http_request` actions only after local validation. Write actions require both a validated `api.safe_write_gate` context and per-action safety constraints. The safe-write gate must accept equivalent safety terms such as `confirm_test_env`, `pre_call_check`, `unique_prefix`, `data_prefix`, `data_isolation`, `cleanup_required`, and `require_cleanup`; it must not depend on one prompt-specific phrase. Follow-up write actions must carry policy/validated inputs forward so equivalent per-action constraints such as `use_unique_prefix` and `cleanup_test_data` are honored while missing gate/isolation evidence is still blocked.
...
- Planner/model placeholders such as `${created_id}` and `${{created_id}}` are equivalent to `{{created_id}}`; runner dependency detection, URL/body/query substitution, and path matching must normalize them before execution.
- Planner-provided HTTP actions may use natural model aliases such as `body_template` for JSON body and `variable_extraction` for response extraction. `api_runner` must normalize these into the same request contract as `body`/`json` and `extract` without requiring product-specific branches.
- Planner-provided JSON bodies may use semantically generic field names (`name`, `id`) while the OpenAPI request body schema uses resource-specific fields (`brandName`, `brandId`). For schema-backed write actions, `api_runner` may repair low-risk field names against the requestBody schema and fill missing required non-ID fields from the schema mock-data generator; POST ID fields should not be invented just because a schema marks them required.
- Synthetic CRUD request bodies must stay schema-aware when applying the unique TestClaw prefix. Prefix injection may replace only schema string/text fields; numeric/boolean ID-like fields such as `id`, `xxxId`, `xxxCode`, and `code` must keep their schema type. For POST create bodies, optional server-managed fields such as IDs, codes, audit timestamps/users, delete flags, and versions should be omitted instead of populated with fake strings.
- Exact documented paths must win over path-template matches when resolving action endpoints. Fixed collection paths such as `/itemBrand/list` must not be treated as `/itemBrand/{id}` or blocked on a synthetic path dependency.
...
- LLM emits a valid safe-write gate using equivalent safety wording (`data_prefix`, `data_isolation`, `cleanup_required`) plus isolated follow-up write actions -> execute the chain; do not block merely because the prompt used different safe-write vocabulary.
- LLM emits a valid CRUD chain using `body_template`, `variable_extraction`, or `${created_id}` placeholders -> normalize and execute the documented request sequence when local policy and safe-write guardrails pass.
- CRUD schema contains optional integer/long `xxxCode` or `id` fields -> generated POST body omits them or preserves numeric type; it must never overwrite them with a TestClaw string prefix.
...
- Regression: `api_runner` executes a safe planner-provided CRUD action chain when `api.safe_write_gate` and follow-up write actions use equivalent safety terms such as `data_prefix`, `data_isolation`, `use_unique_prefix`, and `cleanup_test_data`.
- Regression: `api_runner` normalizes `body_template`, `variable_extraction`, and `${created_id}` placeholders, repairs schema-backed body field names, and still preserves safe-write cleanup requirements.
- Regression: synthetic CRUD POST bodies use schema-aware prefix injection, omit optional server-managed ID/code/audit fields, and preserve numeric `xxxCode`/`id` types.
- Regression: `api_runner` resolves exact action paths before path-template matches so `/resource/list` is not blocked as `/resource/{id}`.
...
- Positive schema-derived requests with path parameters must not invent synthetic IDs such as `1` when no OpenAPI example is present. Prefer executing list/collection endpoints first, extract resource-scoped identifiers from successful JSON responses (for example `warehouseId` from `/wms/warehouse/list` rows), then inject those values into detail endpoints. If no safe upstream value exists, skip the dependent positive request with a clear missing-dependency reason.
- Auth negative probes must use safe schema placeholder values for path parameters because the assertion is the unauthorized boundary, not the resource identity. Do not require list-derived IDs for 401/403 probes, even when the run has runtime auth for positive requests.
...
- Schema has both `GET /wms/warehouse/list` and `GET /wms/warehouse/{id}` with no path example -> execute the list request first, extract `warehouseId` from a successful response, then call `/wms/warehouse/{warehouseId}`. If the list returns no identifier, skip the detail request; do not send `/wms/warehouse/1` as a positive business assertion.
- Auth-required `GET /wms/warehouse/{id}` -> execute the unauthorized probe without auth using a safe placeholder ID such as `1` and pass on HTTP or envelope 401/403. Positive authenticated business assertions still require a real upstream ID or a clear skip.
...
- Good: a WMS-style run derives detail IDs from prior list responses and turns missing upstream IDs into skipped dependencies instead of false backend 500 findings from guessed IDs.
- Bad: skipped write requests or auth-positive checks are counted as failed, producing a false `BUG_FOUND` run.
- Bad: the runner replaces every `{id}` with `1` for positive authenticated detail requests, creating false product failures when the resource does not exist.
...
- Regression: schema-derived positive detail endpoints use list-derived path parameters when available and skip missing dependencies instead of sending synthetic IDs.
- Regression: explicit `depends_on`/`extract` API cases still execute producer requests before dependent consumers after request ordering changes.
- Regression: auth-negative probes still use safe placeholder path params and do not require list-derived IDs, with or without runtime auth for positive cases.
