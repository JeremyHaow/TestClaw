...
- Auth negative probes without credentials may still use safe schema placeholder values for path parameters because the assertion is the unauthorized boundary, not the resource identity. Do not require list-derived IDs for unauthenticated 401/403 probes.
...
- Auth-required `GET /wms/warehouse/{id}` with no credentials -> execute the unauthorized probe using a safe placeholder ID such as `1` and pass on HTTP or envelope 401/403; skip the positive authenticated assertion.
...
- Regression: unauthenticated auth-negative probes still use safe placeholder path params and do not require list-derived IDs.
